...
export default function ProjectDetailPage() {
  ...
  async function duplicateLine(line: ProductLine, event?: React.MouseEvent) {
    ...
    copy.room = `${line.room} kopie`
    copy.sort_order = (lines.length || 0) + 1
  ...
  async function deleteLine(line: ProductLine, event?: React.MouseEvent) {
    event?.stopPropagation()
    const confirmed = confirm(`Productregel verwijderen: ${line.product_type} - ${line.room}?`)
    if (!confirmed) return
  ...
  return (
    <>
      ...
      <div className="card">
        <h2>Productregels</h2>
        <p className="muted">Klik op een regel om te bewerken. Gebruik kopiëren voor meerdere vergelijkbare horren.</p>
        <table className="table">
          ...
          <tbody>
            {lines.map((line) => (
              <tr
                key={line.id}
                onClick={() => router.push(`/projects/${project.id}/product-lines/${line.id}/edit`)}
              ...
              >
                ...
                <td>
                  <div className="row">
                    <button className="button secondary" onClick={(event) => duplicateLine(line, event)}>
                      Kopiëren
                    </button>
                    <button className="button secondary" onClick={(event) => deleteLine(line, event)}>
                      Verwijderen
                    </button>
                  </div>
                </td>
